...
export function DeckViewClient({
  username, deckName, description, rows, allSets,
}: {
  username:    string;
  deckName:    string;
  description: string;
  ...
  return (
    <div style={{ width: "100%", background: BG0 }}>
      ...
      <section style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "32px 24px 16px" }} className="dk-section-header">
        <style>{`@media (min-width: 1024px) { .dk-section-header { padding: 32px 80px 16px !important; } }`}</style>
        ...
        <p style={{ margin: "8px 0 0", fontFamily: MONO, fontSize: "11px", color: INK2, letterSpacing: "0.1em" }}>
          {/* En un deck cuentan las copias: 60 cartas pueden ser 20 distintas */}
          {totalCopias} {totalCopias === 1 ? "carta" : "cartas"}
          {resolved.length > 0 && ` · ${resolved.length} distintas`}
          {!setsLoaded && rows.length > 0 && " · cargando..."}
        </p>
      </section>
      ...
      <section style={{ padding: "32px 24px 80px" }} className="dk-body">
        <style>{`
          @media (min-width: 1024px) { .dk-body { padding: 48px 80px 80px !important; } }
          ...
          @media (max-width: 767px) { .dk-cards-grid { grid-template-columns: repeat(2, minmax(0, 1fr)) !important; gap: 12px !important; } }
        `}</style>

        {!setsLoaded && rows.length > 0 ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "16px" }} className="dk-cards-grid">
            {Array.from({ length: Math.min(rows.length, 12) }).map((_, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px", overflow: "hidden", display: "flex", flexDirection: "column" }}>
                <div className="dk-skeleton" style={{ width: "100%", aspectRatio: "5/7", flexShrink: 0 }} />
                <div style={{ padding: "12px 14px", display: "flex", flexDirection: "column", gap: "10px", flex: 1 }}>
                  <div className="dk-skeleton" style={{ height: "10px", width: "40%" }} />
                  <div className="dk-skeleton" style={{ height: "12px", width: "75%" }} />
                  <div className="dk-skeleton" style={{ height: "18px", width: "56px" }} />
                </div>
              </div>
            ))}
          </div>
                  ...
                  >
                    <img
                      ...
                      onError={e => {
                        ...
                        const ph = t.parentElement?.querySelector(".dk-img-ph") as HTMLElement | null;
                        if (ph) ph.style.display = "flex";
                      }}
                    />
                    <div className="dk-img-ph" style={{ display: "none", position: "absolute", inset: 0, flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "8px", background: "rgba(255,255,255,0.03)" }}>
