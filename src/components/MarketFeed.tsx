...
function PostCard({ item }: { item: FeedItem }) {
  const tcgUrl = `https://www.tcgplayer.com/search/pokemon/product?q=${encodeURIComponent(item.cardName)}`;
  const wa = item.player?.whatsapp_indicativo && item.player?.whatsapp_numero
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 6, flexWrap: "wrap" }}>
            <a href={`/${displayName}`} style={{
              fontFamily: MONO, fontSize: 12, fontWeight: 700,
              color: COURT, textDecoration: "none", letterSpacing: "0.06em",
            }}>
              @{displayName}
            </a>
          ...
          <p style={{
            fontFamily: MONO, fontSize: 10.5, color: "rgba(245,247,251,0.75)",
            margin: "3px 0 0", lineHeight: 1.55, letterSpacing: "0.02em",
          }}>
            ha publicado{" "}
            ...
            <span style={{
              ...
              color: INK2, letterSpacing: "0.08em", verticalAlign: "middle",
            }}>
              {item.versionLabel}
            </span>
            {" "}del set{" "}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <a
            ...
            style={{
              ...
              color: INK0, fontFamily: MONO, fontSize: 10,
          ...
          >
            <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
              <path d="M1 1h9v9M1 10L10 1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
            TCGPlayer
          </a>
          ...
          {wa ? (
            <a
              ...
              style={{
                ...
                padding: "7px 14px", borderRadius: 8,
                background: `${COURT}18`,
                border: `1px solid ${COURT}40`,
                color: COURT, fontFamily: MONO, fontSize: 10,
...
export function MarketFeed() {
  ...
  const loadMore = useCallback(async () => {
    ...
    if (raw.length < PAGE) { hasMoreRef.current = false; setHasMore(false); }
    cursorRef.current = raw[raw.length - 1].created_at;

    // Fetch players
    const uids = [...new Set(raw.map((r: { user_id: string }) => r.user_id))];
    const { data: playerRows } = await supabase
      .from("players")
      .select("user_id, username, photo_url, whatsapp_indicativo, whatsapp_numero, pais")
      .in("user_id", uids);
    const playerMap: Record<string, typeof playerRows extends (infer T)[] | null ? T : never> = {};
    (playerRows ?? []).forEach((p: { user_id: string }) => { (playerMap as Record<string, typeof p>)[p.user_id] = p; });

    // Load set card data
    const setIds = [...new Set(raw.map((r: { set_id: string }) => r.set_id))];
    await loadManySets(setIds);

    const newItems: FeedItem[] = raw.map((r: {
      id: string; card_id: number | string; set_id: string;
      price_cop: number; currency: string; version: string;
      created_at: string; user_id: string;
    }) => {
      const cards = SET_CARDS[r.set_id] ?? [];
      const card  = cards.find(c =>
        String(c.card_number) === String(r.card_id) && c.version === r.version
      ) ?? cards.find(c => String(c.card_number) === String(r.card_id));
      const setInfo = ALL_SETS.find(s => s.id === r.set_id);
      const player = (playerMap as Record<string, {
        username: string; photo_url: string | null;
        whatsapp_indicativo: string | null; whatsapp_numero: string | null; pais: string | null;
      }>)[r.user_id] ?? null;
      ...
      return {
        ...
        versionLabel: getVersionLabel(r.version),
      };
    });
    ...
    <>
      ...
      {!hasMore && items.length > 0 && (
        <p style={{
          ...
          color: INK2, letterSpacing: "0.12em", textTransform: "uppercase",
